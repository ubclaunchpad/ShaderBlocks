...
respondLand :: Request -> Handle -> IO ()
respondLand request handle = do
  let response = Response {version = "HTTP/1.1", statuscode = 200}
  hPutStr handle $ show(response)
  hPutStr handle $ "Testing a Landing Page"
...
handleAccept handle hostname = do 
  ...
  case path request of
    "/landing" -> do
      respondLand request handle
      return ()
    _ -> do
      putStrLn $ "path: " ++ (path request)
...
server = withSocketsDo $ do
  args <- getArgs
  case args of
    ["sanity"] -> putStrLn "Sanity check passed"
    [] -> do 
